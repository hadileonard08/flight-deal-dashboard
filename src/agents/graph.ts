...
async function architectNode(state: typeof ItineraryStateAnnotation.State) {
  ...
  const prompt = `
    ...
    - Daily markdown itinerary with practical activities that match the cabin tier. For each day, on the line immediately after the day heading, include exactly one image placeholder for a specific landmark or activity planned that day, formatted as ![IMAGE: English landmark or activity name]. Choose a specific, well-known place written in English (e.g. "Gyeongbokgung Palace", "Tokyo Skytree", "Senso-ji Temple", "Shibuya Crossing", "Victoria Peak", "Tian Tan Buddha"). If the day's activities are broad, expand the horizon and pick one iconic, representative landmark that is directly related to the day (e.g., for Shinjuku Gyoen National Garden pick "Shinjuku Gyoen National Garden", for Shibuya use "Shibuya Crossing"). The term must be specific enough to find a Wikimedia Commons photo of that exact place or activity. Do not use the destination city name, country name, "flag", "skyline", or generic words as the image term. Do not add any URL inside the placeholder and do not include an IMAGE line for the Flight & Arrival Reality or Weather sections.
...
async function criticNode(state: typeof ItineraryStateAnnotation.State) {
  ...
  const prompt = `
    ...
    5. **Daily Images**: Each of the 5 days must include exactly one ![IMAGE: ...] placeholder immediately after the day heading, naming a specific English landmark or activity directly related to that day's plan. The placeholder must not contain a URL. Reject if any image term is the destination city name, country name, "flag", "skyline", or a similarly generic word (e.g. "Hong Kong", "Hong Kong city", "Hong Kong flag", "flag of Hong Kong"). Also reject if an image term is too vague to find a relevant photo (e.g. "Shinjuku area", "local food market", "downtown Tokyo", "historic district"). The term should be an iconic, well-known place such as "Senso-ji Temple", "Shibuya Crossing", or "Gyeongbokgung Palace" that matches the day's activities.
