...
- **Deal quality is based on the standard CPP formula.**
  ...
  - Cash Price comes from the Travelpayouts affiliate API (real-time Flight Search when approved, otherwise the free Data API `prices_for_dates` for economy), cached per `route/cabin/date`. It prefers the award airline, then falls back to the cheapest cash option for that exact route and date. If live lookup fails, the static estimate table is used.
